...
class AgriNetDataLoaderMS480:
        ...
        for r in ret:
            ...
            if 'image_uri' in r.keys() and '/Processed/' in r['image_uri']:
                ...
                if not ((os.path.isfile('{}/{}'.format(frame_uri, 'Calib.txt')) or
                        ...
                        os.path.isfile('{}/{}'.format(frame_uri, FILE_NAMES['MS480'])) and
    ...
    def open_img_with_registeration_by_fid(self, frame_id):
        ...
        ms480_img = cv2.imread('{}/{}'.format(frame_uri, FILE_NAMES['MS480']), cv2.IMREAD_UNCHANGED)
        ms480_img = cv2.rotate(ms480_img, cv2.ROTATE_90_CLOCKWISE)
        ...
        reg_rgb_fix = cv2.warpAffine(rgb_img, reg_param.T['MS480'],
                                     dsize=(ms480_img.shape[1], ms480_img.shape[0]),
                                     flags=cv2.WARP_INVERSE_MAP)
        reg_rgb_fix = cv2.resize(reg_rgb_fix, dsize=None, fx=0.9, fy=0.9, interpolation=cv2.INTER_CUBIC)
        ms480_img = cv2.resize(ms480_img, dsize=None, fx=0.9, fy=0.9, interpolation=cv2.INTER_CUBIC)
        h, w = ms480_img.shape[0:2]
        dw = 384
        dh = 288
        ...
        ms480_img = ms480_img[yc - dup:yc + ddown, xc - dleft:xc + dright]
        if self.CNT <= 25:
            cv2.imwrite('RGB_{}.jpg'.format(self.CNT), reg_rgb_fix)
            ms480_img = transform_ir(ms480_img)
            cv2.imwrite('MS480_{}.jpg'.format(self.CNT), ms480_img)
            self.CNT += 1
        else:
            exit(0)
        return reg_rgb_fix, ms480_img
    ...
    def load_data_with_registeration(self, max_size=100, thermal_vis=True, testing=0.1):
        for i, frame_id in enumerate(self.elbit2_processed_frame_ids_with_uri.keys()):
            ...
            cv2.imwrite('{}/{}/{}_{}'.format(self.output_dir.absolute(), dst, frame_id, FILE_NAMES['MS480']),
                        depth_image)
            if thermal_vis:
                thermal_img_vis = transform_ir(depth_image)
                cv2.imwrite('{}/vis/{}_vis_{}'.format(self.output_dir.absolute(), frame_id, FILE_NAMES['MS480']),
                            thermal_img_vis)


if __name__ == '__main__':
    agrinet_dl = AgriNetDataLoaderMS480('./EXP_523_MS480').connect_to_agrinet()
